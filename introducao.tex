...
\begin{itemize}
 ...
 \item Obtenção das sugestões de modificações (correção de defeitos, melhorias, novas funcionalidades) e alterações (\textit{commit}) no código das aplicações selecionadas por meio do acesso a seus repositórios de código.
 \item Seleção das avaliações dos usuários, sugestões de modificações e alterações que abordam aspectos da acessibilidade dos aplicativos selecionados.
 \item Analise das avaliações, das sugestões de modificações e alterações relacionadas à acessibilidade das aplicações.
 \end{itemize}
